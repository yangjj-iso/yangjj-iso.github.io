...
用户发来一句话，后端调用一次大模型，然后把结果返回：

{% include diagram.html src="simple-chat.svg" alt="一次问答：浏览器请求到服务端，再调用模型返回响应" %}

对于一个几秒钟就能完成的问答，这种模型没有任何问题。

但当 Agent 开始承担更复杂的任务，事情会迅速发生变化。
...
如果执行过程中 Agent 发现信息不足，它还可能暂停下来：

> 请补充一下完整的岗位描述。

用户可能两小时之后才回来。

到这里，一个原本普通的 Web 请求，实际上已经变成了一个 **Long-running Workflow**。

而很多 Agent 系统最早遇到的工程问题，正是因为我们仍然在用“普通 HTTP 请求”的方式理解它。
<!--more-->
...
也就是说，只要这个请求出了问题，Agent 的执行就可能跟着出问题。
...
真正执行 Agent 的，是独立 Worker。

这里有一个很容易被忽略的区别：

> `async/await` 解耦的是线程等待，任务队列解耦的是服务生命周期。

即使一个 FastAPI Handler 是 `async` 的，只要你还在：

```python
await agent.run()
```

Agent 仍然属于这次 Request。

而进入 Queue 后：

{% include diagram.html src="queue-decouple.svg" alt="API 入队后即可结束，Worker 独立领取 task_id 再跑 Agent" %}

API 和 Agent 执行终于变成了两个独立生命周期。

这才是任务队列对于 Agent 系统真正重要的地方。
...
> 任务已经放进 Redis 了，那系统是不是就可恢复了？

答案仍然是否定的。

Queue 能告诉我们：`task_123` 需要执行。
...
所以一个真正可恢复的 Agent 系统，需要把状态拆成几个不同维度。

### 1. Task State：这个任务现在是什么状态？

最基础的是业务任务本身，例如 `PENDING → RUNNING → COMPLETED`。Agent 场景通常还需要 `WAITING`、`FAILED`、`CANCELLED`。最终可能形成：
...
> Agent 没有失败，只是在等待用户提供新的信息。

这类状态最好持久化在数据库中，而不是依赖 Worker 内存。

因为一个任务可能上午十点创建，十点零二分等待用户，下午两点半才重新继续。

业务任务活了四个半小时，但 Worker 并不应该被占用四个半小时。

---

### 2. Execution State：Agent 到底执行到哪里？

任务状态告诉我们它在 `RUNNING`，但没有回答：
...
Checkpoint 保存的不是简单的 `RUNNING`，而是类似：
...
这可能是 Agent 恢复中最危险的问题。

假设 Agent 调用了一个有副作用的工具：`create_report`、`send_email`、`charge_user`、`write_database`。

流程是：

{% include diagram.html src="side-effect-crash.svg" alt="Tool 已经成功、外部系统已变化，结果还没记下 Worker 就崩溃" %}

这时候新 Worker 恢复以后，如果只是“再执行一次”，可能产生重复副作用。

所以真正的可恢复系统不能只做 Retry。

还需要：

{% include diagram.html src="idempotency-ledger.svg" alt="幂等与 Tool Execution Ledger 一起防止重复副作用" %}

每一次 Tool Call 都应该有稳定身份，例如 `tool_call_id = abc123`。

- 执行前：`claim abc123`
- 执行成功：`abc123 = completed`
- 恢复时：问 `abc123` 是否已经执行？

如果已经完成，就不应该再次产生同样的副作用。

这也是为什么在分布式系统里，一个更加现实的目标通常不是追求绝对的 exactly-once，而是：

> at-least-once delivery + idempotent execution

也就是说：

> 消息可以重复到达，但业务动作不能随便重复发生。

---

### 4. Event State：用户漏掉了哪些执行过程？

Agent 系统通常还需要实时展示：正在分析、正在搜索、调用 Tool、正在生成报告。

最简单的方式是 Agent 直接推 SSE 到浏览器。但这意味着浏览器一断线，中间事件就丢了。因此更加可靠的链路应该先写 durable event log：

{% include diagram.html src="sse-vs-outbox.svg" alt="左边 Agent 直推 SSE 会丢事件，右边先写 Event Outbox 再推送" %}

Agent 每产生一个事件，都先写入一个 durable event log。例如：
...
浏览器保存 `last_seq = 105`。如果网络断开，Agent 仍然可以继续运行，事件变成 106、107、108、109。用户回来后只需要带上 `after_seq = 105`，服务端重新读取 `seq > 105` 并继续推送。

这时候 SSE 的定位就变得非常清楚：

> **SSE 只是 Delivery Channel，Outbox 才是 Event Source of Truth。**

断线意味着“实时连接断了”，而不是“任务历史消失了”。
...
| --- | --- |
| Queue | 谁来执行 |
| Task Database | 任务现在是什么状态 |
| Checkpoint | Agent 执行到哪里 |
| Tool Ledger | 某个副作用是否已经发生 |
| Event Outbox | 执行过程中发生过什么 |
| SSE Replay | 客户端错过了哪些事件 |
| Idempotency Key | 这是不是同一次请求 |

如果我们把这些东西全塞进一个 Redis Queue，就会很快失控。

比较健康的边界反而是：

{% include diagram.html src="durability-vs-queue.svg" alt="PostgreSQL 负责持久，Redis Queue 只负责调度和协调" %}

Queue 只需要知道：`task_123` 该执行了。

至于它属于谁、输入是什么、状态是什么、有没有等待用户、报告生成到哪里，这些仍然由 durable storage 保存。

---

一个很典型的 Agent 场景是 HITL，也就是 Human in the Loop。

假设 Agent 运行到一半发现：

> 我需要用户提供完整的合同内容。

此时最糟糕的做法是让 Worker 空等三个小时。正确方式是把任务放回 `WAITING`，交还 Worker Slot：
...
> 系统是否知道任务之前发生了什么，以及重新启动以后应该从哪里继续。

---

## 四、真正困难的不是异步，而是故障边界

如果只是“把任务丢进 Redis”，其实并不难。
...
假设第二步成功以后，API 在第三步之前崩溃：

{% include diagram.html src="pg-without-job.svg" alt="PostgreSQL 里任务已是 PENDING，Redis 里却没有对应 job" %}

这时候数据库里明明有任务，却永远没人执行。

一个常见的改进方式是 **Transactional Outbox**。

在同一个数据库事务中同时写 EvaluationTask 和 DispatchOutbox：
...
这样即使 API 在提交事务后立刻崩掉，`dispatch_outbox` 仍然存在。系统启动以后可以继续把任务送进 Queue。
...
我们经常会看到 `LLM Retry = 3`、`Tool Retry = 3`、`Job Retry = 3`。最后一次普通网络波动，可能放大成十几次外部调用。

所以 Retry 也应该分层。

{% include diagram.html src="retry-layers.svg" alt="Capability Retry 管单次外部调用，Job Retry 管整个 Worker Job" %}

哪些错误能重试，必须明确分类。因为对 `GET search result` 重复可能没什么，但 `charge card`、`send email`、`create order` 就完全不是同一个问题。

所以在一个可恢复 Agent 系统里：

> **Idempotency 往往比 Retry 更重要。**

---

## 五、可恢复，不等于永不失败

我觉得这是设计 Agent 系统时非常重要的一个认知。
...
对于一个长时间运行的 Agent，我认为至少应该回答清楚下面几个问题：
...
- 浏览器断线怎么办？
- Agent 等待用户怎么办？
- Tool 已经成功但 Worker 挂了怎么办？
- 任务状态已经入库但 Job 没有成功入队怎么办？
...
> **幂等与 Ledger 负责副作用尽量不重复。**

> **Event Outbox 与 Replay 负责客户端断线以后仍然能够恢复视图。**

Agent 系统真正需要解耦的，从来不只是线程。

而是生命周期。

HTTP Request、业务 Task、Worker Job、Agent Run 和 SSE Connection，都应该允许独立失败，也应该能够独立恢复。

只有做到这一点，Agent 才开始从一个“长时间占着请求的 LLM 调用”，真正变成一个可以被后端系统可靠管理的任务。
