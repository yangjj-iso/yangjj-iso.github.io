...
但真正进入复杂交易系统以后，会发现最难的问题并不是“怎么让几个 RPC 同时成功”。真正的问题是：网络超时、进程崩溃、消息重复，甚至正向流程和回滚流程同时发生以后，系统还能不能判断这笔交易应该往哪个方向走，并让所有资源最终收敛。
...
所以工程上的核心问题也发生了变化。最开始我们关心的是“所有操作能不能在同一瞬间成功或失败”；后来更重要的问题变成了“即使中途发生部分成功，系统能不能在之后恢复到一个唯一、正确、可解释的最终状态”。
...
此时最大的问题已经不是“事务 API 怎么写”，而是调用结果与真实资源状态可能发生分离。下面这个场景最典型。
...
这也揭示了分布式事务最典型的困难：**RPC 的返回结果，不等于资源的最终事实。**
...
对于 A 来说，这三种情况在表面上都可能只是同一个 `timeout`。

所以一个成熟的交易系统必须接受一个基本事实：**分布式系统中存在“未知状态”。**
...
当系统里同时存在订单、券、额度、资金渠道时，很快就会遇到一个问题——**到底应该相信谁？**
...
这时候，分布式事务的结构就不再只是“多个调用组成的一条链”，而是**一个核心业务方向，加上多个跟随它收敛的资源状态机**。
...
最直觉的补偿逻辑，是把正向步骤倒过来执行。图里展示的就是这种思路。
...
所以补偿流程最重要的事情不是马上释放外围资源，而是**先固定整笔交易的方向**。
...
这背后的原则很通用：**先确定核心状态，再让外围资源跟随核心状态收敛。**
...
如果业务动作没有稳定的幂等身份，系统就很难判断这到底是一次新的业务操作，还是同一次操作的技术重试。

一个典型做法，是为一次业务尝试生成稳定的 `idempotency_key`，并让之后的技术重试复用这个身份。
...
这里要区分两个概念：**并发控制解决“两个不同操作同时修改资源”，幂等解决“同一个操作被执行多次”。**
...
于是整个系统的安全性，不再依赖某个进程始终不出错，而是依赖一个更稳定的约束：**任何流程想继续推进，都必须先证明当前状态允许它这么做。**
...
所以更准确的说法是：**最终一致是一种结果，收敛机制才是产生这个结果的原因。**

---

## 十、一个更完整的交易恢复模型

把上面的机制放在一起，可以得到下面这个比较通用的恢复模型。
...
如果只看正常流程，分布式事务其实很简单，图的上半部分已经足够概括。
...
系统设计真正要回答的问题，是在这些最糟糕的故障窗口里，下一次执行还能不能根据持久化事实做出正确决策。
...
本地事务给人的安全感来自一个很强的承诺：**要么全部发生，要么全部不发生。**

而在复杂的分布式业务里，更现实的目标是：**允许中间态存在，但中间态必须可识别；允许部分成功发生，但部分成功必须可恢复；允许请求重复，但重复执行不能破坏业务不变量。**
...
最后可以把整篇文章压缩成一句话：**分布式事务真正要解决的，不是让所有参与者在同一时刻看起来像一个本地事务，而是在不可避免的超时、重试、并发和部分失败之后，仍然让所有资源走向同一个正确的业务终态。**
